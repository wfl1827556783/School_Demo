...
void FindJournal(){
	FILE* fp;
	if ((fp = fopen("D:\\Code\\C\\School_Demo\\SourceFile\\Journal.txt", "r")) == NULL) {
		puts("文件打开失败！");
		return 0;
	}
	else {
		puts("文件打开成功！");
		fp = fopen("D:\\Code\\C\\School_Demo\\SourceFile\\Journal.txt", "r");

		puts("=================================");
	      }
                   }
	FUND ReadFundFromFile(int FID) {

}
	void BuyFund(int FID, USER * user, int buyAmount) {
